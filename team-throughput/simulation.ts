...
<head>
  ...
  <title>Software Release Pipeline Simulation</title>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; }
    body {
      margin: 0;
      min-height: 100vh;
      background: radial-gradient(circle at top left, #1d2b53 0, #0a0f1f 42%, #050812 100%);
      color: #dbeafe;
    ...
    h1 { margin: 0 0 6px; font-size: clamp(1.45rem, 2.4vw, 2.35rem); letter-spacing: -0.04em; }
    p { margin: 0; color: #93a4bd; line-height: 1.45; max-width: 820px; }
    .controls {
      display: grid;
    ...
    .value { color: #f8fafc; font-weight: 800; font-variant-numeric: tabular-nums; }
    .board {
      position: relative;
      overflow: hidden;
    ...
    svg { display: block; width: 100%; height: auto; min-height: 650px; }
    .panel { fill: rgba(15, 23, 42, 0.74); stroke: rgba(148, 163, 184, 0.36); stroke-width: 1.5; }
    ...
    @keyframes spinDash { to { stroke-dashoffset: -32; } }
    .metrics { display: grid; grid-template-columns: repeat(9, 1fr); gap: 8px; padding: 12px; border-top: 1px solid rgba(148, 163, 184, 0.18); }
    ...
    .model-explanation h2 { margin: 0 0 10px; color: #f8fafc; font-size: 1.05rem; }
    .model-explanation p { max-width: 1060px; margin: 0 0 10px; }
    .model-explanation p:last-child { margin-bottom: 0; }
...
<body>
  <main>
    ...
    <section class="controls" aria-label="Simulation controls">
      ...
      <label>Staging defect rate / change <span class="value" id="stagingDefectValue"></span><input id="stagingDefect" type="range" min="0" max="15" step="0.1" value="8" /></label>
      <label>Production defect rate / change <span class="value" id="productionDefectValue"></span><input id="productionDefect" type="range" min="0" max="15" step="0.1" value="3" /></label>
      ...
      <label>Restart simulation <span class="value">clear state</span><button id="reset" type="button">Reset</button></label>
    </section>

    <section class="board">
      <svg id="pipeline" viewBox="0 0 1400 720" role="img" aria-label="Animated release pipeline">
        <defs>
      ...
      <div class="metrics">
        ...
        <div class="metric"><div>Production Fires</div><div id="prodFiresMetric">0</div></div>
        <div class="metric"><div>Avg Prod Size</div><div id="avgProdSizeMetric">0.0</div></div>
        <div class="metric"><div>Fires/Prod. Deployment</div><div id="firesPerProdMetric">0.00</div></div>
        <div class="metric"><div>Released</div><div id="releasedMetric">0</div></div>
      </div>
    </section>
    <div class="legend">
      <span><span class="dot" style="background:#67e8f9"></span>moving through PR CI</span>
    ...
    <section class="model-explanation">
      ...
      <p>This simulation treats each colored block as one change created by one developer. Pull requests are independent: CI, review feedback, and PR fixes happen per change, so one bad PR does not stop unrelated PRs from continuing through the workflow.</p>
      <p>Staging represents the shared integration environment. It keeps accepting new changes while defects are waiting to be fixed, so work can keep piling up there. When staging finds a defect, the bad block stays red in staging and the responsible developer creates a red fix block with the same number. Fix blocks still flow through CI and main, but they do not fail CI. Production cannot run until every red staging block has its matching fix arrive back in staging.</p>
      <p>Production verification models a rollback/fire. If production finds a defect, the deployment moves back to staging, the responsible developers create fixes, and the production fire means everyone stops working on new changes until the next successful production deployment. Blocks that have already passed staging or production are shown light green and will not fail that same stage again.</p>
    </section>
  </main>
  ...
  <script>
    ...
    const coords = {
      ci: { x: 390, y: 82, w: 260, h: 510 },
      main: { x: 720, y: 248, w: 180, h: 168 },
      staging: { x: 970, y: 112, w: 185, h: 438 },
      production: { x: 1210, y: 112, w: 155, h: 438 }
    };

    const devs = Array.from({ length: 5 }, (_, index) => ({
      id: index,
    ...
    const colors = ['#67e8f9', '#93c5fd', '#a7f3d0', '#fde68a', '#f0abfc'];
    const state = {
      nextId: 1,
      blocks: [],
      main: [],
    ...
    function text(value, x, y, className, parent) {
      const node = el('text', { x, y, class: className }, parent);
    ...
    function drawStatic() {
      ...
      el('path', { d: 'M 1158 350 C 1178 350, 1192 350, 1206 350', class: 'lane' }, staticLayer);
      el('circle', { cx: coords.ci.x + coords.ci.w / 2, cy: coords.ci.y + 272, r: 72, class: 'ci-runner' }, staticLayer);
    ...
    function makeVisualBlock(block) {
      const g = el('g', {}, blockLayer);
      const r = el('rect', { width: 25, height: 19, rx: 4, class: 'block', fill: colors[block.devId] }, g);
      const label = el('text', { x: 12.5, y: 13.5, 'text-anchor': 'middle', class: 'block-label' }, g);
    ...
    function setBlockPosition(block, x, y) {
      block.x = x;
      block.y = y;
      if (block.node) block.node.setAttribute('transform', 'translate(' + x + ' ' + y + ')');
    }

    function moveTo(block, x, y, duration, onDone) {
      block.move = { sx: block.x, sy: block.y, tx: x, ty: y, start: performance.now(), duration, onDone };
    }
    ...
    function startStaging(now) {
      if (state.stagingBusy || state.productionBusy || state.main.length === 0) return;
      state.stagingBusy = true;
      state.stagingRuns++;
      const batch = state.main.splice(0, state.main.length);
    ...
    function startProduction(batch) {
      state.productionBusy = true;
      state.prodRuns++;
      state.totalProdChanges += batch.length;
    ...
    function createBatchBadge(label, target, count, color) {
      const g = el('g', {}, batchLayer);
      el('rect', { x: target.x + 18, y: target.y + target.h - 76, width: target.w - 36, height: 48, rx: 13, fill: 'rgba(15, 23, 42, 0.78)', stroke: color, 'stroke-width': 1.4 }, g);
      const t = text(label, target.x + 30, target.y + target.h - 48, 'panel-subtitle', g);
      t.setAttribute('fill', '#fff');
      const c = text(count + ' changes', target.x + 30, target.y + target.h - 30, 'queue-label', g);
    ...
    function releaseBatch(batch) {
      ...
      state.released += batch.length;
    }

    function updateControls() {
      const bind = (id, prop, scale, suffix) => {
        const input = document.getElementById(id);
        const value = document.getElementById(id + 'Value');
        const apply = () => {
          config[prop] = Number(input.value) / scale;
          value.textContent = input.value + suffix;
        };
        input.addEventListener('input', apply);
        apply();
      };
      bind('ciSuccess', 'ciSuccessProbability', 100, '%');
      bind('stagingDefect', 'stagingDefectRate', 100, '%');
      bind('productionDefect', 'productionDefectRate', 100, '%');
      const rate = document.getElementById('produceRate');
      const rateValue = document.getElementById('produceRateValue');
      const applyRate = () => { config.productionRate = Number(rate.value); rateValue.textContent = rate.value + '/10'; };
    ...
    function reset() {
      ...
      state.stagingRuns = 0;
      state.prodRuns = 0;
      state.productionFires = 0;
      state.totalProdChanges = 0;
    ...
    function updateMetrics(now) {
      ...
      document.getElementById('stagingMetric').textContent = state.stagingRuns;
      document.getElementById('prodMetric').textContent = state.prodRuns;
      document.getElementById('prodFiresMetric').textContent = state.productionFires;
      document.getElementById('avgProdSizeMetric').textContent = state.prodRuns === 0 ? '0.0' : (state.totalProdChanges / state.prodRuns).toFixed(1);
      document.getElementById('firesPerProdMetric').textContent = state.prodRuns === 0 ? '0.00' : (state.productionFires / state.prodRuns).toFixed(2);
    ...
    function tick(now) {
      ...
      state.blocks.forEach((block) => {
        if (block.move) {
          const p = Math.min(1, (now - block.move.start) / block.move.duration);
          const e = ease(p);
          setBlockPosition(block, block.move.sx + (block.move.tx - block.move.sx) * e, block.move.sy + (block.move.ty - block.move.sy) * e);
          if (p === 1) {
            const done = block.move.onDone;
            block.move = null;
            if (done) done();
          }
        }
        if (block.status === 'ci' && now >= block.ciDoneAt) {
          if (block.isFix || Math.random() < config.ciSuccessProbability) mergeToMain(block);
...
console.log("Wrote docs/team-throughput/index.html");
